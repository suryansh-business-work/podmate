...
describe('SafeImage', () => {
  ...
  it('renders fallback View with icon when uri is null', () => {
    const { getByLabelText } = render(<SafeImage uri={null} style={{ width: 100, height: 100 }} />);
    expect(getByLabelText('Placeholder image')).toBeTruthy();
  });

  it('renders fallback View when uri is empty string', () => {
    const { getByLabelText } = render(<SafeImage uri="" style={{ width: 100, height: 100 }} />);
    expect(getByLabelText('Placeholder image')).toBeTruthy();
  });
  ...
  it('uses default fallback icon "person" when not specified', () => {
    const { getByText } = render(<SafeImage uri="" style={{ width: 50, height: 50 }} />);
