...
const LoginForm: React.FC<LoginFormProps> = ({ onLogin, error, setError, setSnackMessage }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, { setSubmitting }) => {
      try {
        setError('');
        const { data } = await adminLogin({ variables: { email: values.email, password: values.password } });
    ...
    <Box sx={{ width: '100%', maxWidth: 420 }}>
      <Box sx={{ mb: 5 }}>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5, mb: 4 }}>
          <Box sx={{ width: 48, height: 48, borderRadius: 2.5, background: 'linear-gradient(135deg, #FF3370, #F50247)', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 4px 16px rgba(245,2,71,0.3)' }}>
            <Typography sx={{ color: '#FFF', fontSize: 26, display: 'flex' }}>🎉</Typography>
          </Box>
          <Typography variant="h6" sx={{ fontWeight: 700, letterSpacing: -0.3 }}>PartyWings</Typography>
        </Box>
        <Typography variant="h4" sx={{ fontWeight: 800, fontSize: { xs: 28, sm: 32 }, letterSpacing: -0.5, mb: 1 }}>
          Welcome back
        </Typography>
      ...
      <form onSubmit={formik.handleSubmit}>
        <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.primary', letterSpacing: 0.5, mb: 0.5, display: 'block' }}>EMAIL</Typography>
        <TextField
          name="email" type="email" placeholder="admin@partywings.com"
        ...
        <Typography variant="caption" sx={{ fontWeight: 600, color: 'text.primary', letterSpacing: 0.5, mb: 0.5, display: 'block' }}>PASSWORD</Typography>
        <TextField
        ...
        <Button type="submit" variant="contained" fullWidth size="large"
          ...
          sx={{
            ...
            py: 1.6, fontSize: 15, fontWeight: 700, mb: 2,
            boxShadow: '0 4px 20px rgba(245,2,71,0.35)',
            '&:hover': { background: 'linear-gradient(135deg, #C7003A 0%, #F50247 100%)', boxShadow: '0 6px 24px rgba(245,2,71,0.45)' },
          }}
        >
          {formik.isSubmitting ? <CircularProgress size={24} color="inherit" /> : 'Sign In'}
        </Button>

        <Button variant="text" fullWidth size="small" onClick={handleSendCredentials} disabled={sendingCredentials}
          sx={{ color: 'text.secondary', fontSize: 13, '&:hover': { bgcolor: 'rgba(245,2,71,0.04)' } }}
        >
      ...
      <Typography variant="caption" color="text.secondary" sx={{ display: 'block', textAlign: 'center', mt: 5, fontSize: 12 }}>
