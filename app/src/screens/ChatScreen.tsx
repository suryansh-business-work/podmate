...
const ChatRoom: React.FC<ChatRoomProps> = ({ pod, onBack }) => {
  ...
  const handleSend = useCallback(async () => {
    ...
    setMessageText('');
    try {
      await sendMessageMutation({ variables: { podId: pod.id, content: trimmed } });
...
const styles = StyleSheet.create({
  ...
  inputBar: { flexDirection: 'row', alignItems: 'flex-end', paddingHorizontal: spacing.md, paddingVertical: spacing.sm, borderTopWidth: 1, borderTopColor: colors.surfaceVariant, backgroundColor: colors.white, gap: spacing.sm },
