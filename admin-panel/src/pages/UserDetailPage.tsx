  ...
  return (
    <Box>
      <Breadcrumbs sx={{ mb: 2 }}>
        ...
        <Link underline="hover" color="inherit" sx={{ cursor: 'pointer' }} onClick={() => navigate('/users')}>
          Users
        </Link>
        <Typography color="text.primary" fontWeight={600}>{user.name}</Typography>
      </Breadcrumbs>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, md: 4 }}>
          <Card>
            <CardContent sx={{ textAlign: 'center', py: 4 }}>
              ...
              <Typography variant="body2" color="text.secondary" mb={1}>{user.phone}</Typography>
              <Box display="flex" justifyContent="center" gap={1} mb={1}>
                <Chip label={user.role.replace('_', ' ')} size="small" color={user.role === 'ADMIN' ? 'error' : user.role === 'PLACE_OWNER' ? 'warning' : 'default'} />
                <Chip label={user.isActive ? 'Active' : 'Disabled'} size="small" color={user.isActive ? 'success' : 'error'} variant="outlined" />
              </Box>
              {user.isVerifiedHost && (
                <Chip icon={<VerifiedIcon />} label="Verified Host" color="success" size="small" variant="outlined" />
              )}
            </CardContent>
          </Card>
        </Grid>
        ...
        <Grid size={{ xs: 12, md: 8 }}>
          <Card>
            <CardContent>
              <Typography variant="h6" mb={2}>Information</Typography>
              <Divider sx={{ mb: 1 }} />
              ...
              {!user.isActive && (
                <>
                  <Divider sx={{ my: 1 }} />
                  <InfoRow label="Disable Reason" value={<Typography variant="body2" color="error">{user.disableReason}</Typography>} />
                </>
              )}
            </CardContent>
          </Card>
